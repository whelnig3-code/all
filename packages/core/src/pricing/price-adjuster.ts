...
import { calculateWholesalePrice, toPsychPrice } from './wholesale'
...
export function adjustPrice(
  ...
  if (undercutPrice < minMarginResult.salePrice) {
    ...
    if (undercutPrice > targetResult.salePrice && lowestCompetitorPrice > currentPrice) {
      ...
      // 실제 언더컷 가격 사용
      // 심리가격 적용: X,900원 패턴
      // 예) undercutPrice 21,990 → toPsychPrice(21,991) = 21,900원
      candidatePrice = toPsychPrice(undercutPrice + 1)
    }
  }
