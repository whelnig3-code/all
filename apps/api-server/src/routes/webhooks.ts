...
import { orderQueue, type OrderJobData, talkTalkQueue, type TalkTalkJobData } from '../queues'
import { naverWebhookSchema, talkTalkWebhookSchema } from '../schemas'
...
export const webhooksRouter: FastifyPluginAsync = async (fastify) => {
  /**
   * POST /webhooks/talktalk
   * 톡톡 메시지 웹훅 수신
   *
   * 비유: 전화 교환원 — MESSAGE_RECEIVED + 메시지 있을 때만 상담원(큐)에 연결
   */
  fastify.post('/talktalk', {
    config: { rateLimit: { max: 50, timeWindow: '1 minute' } },
  }, async (request, reply) => {
    const parsed = talkTalkWebhookSchema.safeParse(request.body)
    if (!parsed.success) {
      logger.warn('톡톡 웹훅 payload 검증 실패', { body: request.body })
      return reply.code(400).send({ error: '유효하지 않은 톡톡 웹훅 데이터' })
    }

    const { eventType, channelId, customerId, message, messageType } = parsed.data

    // MESSAGE_RECEIVED + 메시지 있을 때만 큐에 추가
    if (eventType === 'MESSAGE_RECEIVED' && message) {
      await talkTalkQueue.add('process-message', {
        channelId,
        customerId,
        message,
      ...
      logger.info('톡톡 메시지 큐 추가', { channelId, customerId })
    }
