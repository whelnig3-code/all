...
/** 환율 갱신 큐 (Phase 5 — 구매대행) */
export const exchangeRateQueue = new Queue('exchange-rate', defaultQueueOptions)

/** 도매처 자동 주문 큐 */
export const wholesaleOrderQueue = new Queue('wholesale-order', defaultQueueOptions)

/** 운송장 번호 폴링 큐 */
export const trackingPollQueue = new Queue('tracking-poll', defaultQueueOptions)

/** 일일 리포트 큐 (Phase C — anomaly-detector 연결) */
export const dailyReportQueue = new Queue('daily-report', defaultQueueOptions)

/** 리뷰 모니터링 큐 (Phase C — 리뷰 수 추적 + 부스트 모드) */
export const reviewMonitorQueue = new Queue('review-monitor', defaultQueueOptions)
...
/** 일일 리포트 작업 데이터 (Phase C) */
export interface DailyReportJobData {
  /** 리포트 대상 날짜 (YYYY-MM-DD) */
  targetDate: string
  /** 운영 계정 ID */
  accountId: string
}

/** 리뷰 모니터링 작업 데이터 (Phase C) */
export interface ReviewMonitorJobData {
  productId: string
  naverProductId: string
  accountId: string
}
