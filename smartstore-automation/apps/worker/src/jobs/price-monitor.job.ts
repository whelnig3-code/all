...
import { adjustPrice } from '@smartstore/core'
  ...
  const worker = new Worker<PriceMonitorJobData>(
    ...
    async (job: Job<PriceMonitorJobData>) => {
      ...
      try {
        ...
        // 2. 경쟁가 수집 — NaverShoppingCrawler (Playwright, robots.txt 자동 준수)
        const competitorPrices = await naverShoppingCrawler.fetchCompetitorPrices(
          product.name,
          PRICE_STRATEGY.maxCrawlResults,
        )

        if (competitorPrices.length === 0) {
          logger.info('경쟁 상품 없음, 가격 유지', { productId })
          return { action: 'no_change', reason: '경쟁 상품 없음' }
        }

        // 3. 경쟁가 DB 저장 (크롤러에서 rank 직접 제공)
