...
export const adminRouter: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/system', async (_req, reply) => {
    ...
    const response = {
      ...
      competitorQueueDepth: 0, // Worker 프로세스에서 Redis key 'worker:queueDepth'로 공유 예정
  ...
  // =============================================
  // GET /admin/alerts — 최근 알림 (JobLog 기반)
  // =============================================
  fastify.get('/alerts', async (_req, reply) => {
    try {
      const alerts = await prisma.jobLog.findMany({
        where: {
          OR: [
            { status: 'failed' },
            { jobType: { in: ['order_received', 'price_adjusted', 'stock_warning', 'approval_timeout'] } },
          ],
        },
      ...
      return reply.send({
        alerts: alerts.map((a) => ({
          ...
          jobType: a.jobType,
          status: a.status,
          message: a.error || `${a.jobType} ${a.status}`,
          createdAt: a.startedAt,
        })),
      })
    } catch (error) {
      logger.error('alerts 쿼리 실패', { error })
      return reply.code(500).send({ error: 'alerts query failed', detail: String(error) })
    }
  })
