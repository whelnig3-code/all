    ...
    async (job: Job<RegistrationJobData>) => {
      ...
      try {
        ...
        const registrationResult = await registerProductToNaver({
          ...
          images: finalImages,
          description: buildDetailHtml(finalImages, product.description ?? ''),
 ...
 */
export function buildDetailHtml(imageUrls: string[], originalDescription: string): string {
  if (imageUrls.length === 0) return originalDescription

  // 대표 이미지 + 추가 이미지를 상단에 배치
  const imageHtml = imageUrls
    .map((url) => `<img src="${url}" alt="상품 이미지" style="width:100%;max-width:1000px;display:block;margin:0 auto 8px;" />`)
    .join('\n')
