  ...
  const worker = new Worker<OrderJobData>(
    ...
    async (job: Job<OrderJobData>) => {
      ...
      try {
        ...
        // 7. 도매 자동 발주 (AUTO_WHOLESALE_ORDER_ENABLED)
        if (config.autoWholesaleOrderEnabled && product.source && product.sourceProductId) {
          await wholesaleOrderQueue.add(
            'place-order',
            {
              orderId: order.id,
              naverOrderId: job.data.naverOrderId,
              ...
              shippingAddress: {
                ...
                zipCode: orderItem?.shippingAddress?.zipCode ?? '',
              },
            },
            { jobId: `wholesale-${order.id}` },
          )
          logger.info('도매 자동 발주 큐 추가', { orderId: order.id, source: product.source })
        }
