...
import type { DomaeggukOrdererOptions } from './order-types'

const logger = createLogger('domaegguk-orderer')
const DOMAEGGUK_BASE_URL = 'https://domeggook.com'

export class DomaeggukOrderer implements WholesaleOrderer {
  ...
  constructor(
    ...
    options?: DomaeggukOrdererOptions,
  ) {
   ...
   * 1. 브라우저 초기화
   * 2. 로그인 페이지 이동
   * 3. credentials 입력 + 로그인 버튼 클릭
   * 4. 캡차 감지 시 에러 throw
   * 5. 로그인 성공 여부 확인
   * 6. storageState 저장 (쿠키 캐시)
   */
  async login(): Promise<void> {
    ...
    logger.info('도매꾹 로그인 시도', { username: this.username })
    await page.goto(`${DOMAEGGUK_BASE_URL}/login`, {
    ...
    // credentials 입력
    await page.fill('input[name="id"], #id, input[type="text"][name="userId"]', this.username)
    await page.fill('input[name="password"], #password, input[type="password"]', this.password)
    await page.click('button[type="submit"], .login_btn, #loginBtn')
    ...
    // storageState 저장 (쿠키 캐시)
    if (this.context) {
      await this.context.storageState()
    }

    this.isLoggedIn = true
    logger.info('도매꾹 로그인 성공')
  }
  ...
  async placeOrder(request: WholesaleOrderRequest): Promise<WholesaleOrderResult> {
    try {
      ...
      // 상품 페이지 이동
      await page.goto(`${DOMAEGGUK_BASE_URL}/products/${sourceProductId}`, {
        waitUntil: 'domcontentloaded',
        timeout: 15000,
      })
      await this.randomDelay()
      ...
      // 수량 입력
      await page.fill('input[name="quantity"], #quantity, .quantity-input', String(quantity))
      ...
      // 구매하기 클릭
      await page.click('button.buy_btn, #buyBtn, .purchase-button')
      await this.randomDelay()

      // 배송지 입력 (개인정보 로그 기록 금지)
      await page.fill('input[name="receiverName"], #receiverName', shippingAddress.name)
      await page.fill('input[name="receiverPhone"], #receiverPhone', shippingAddress.phone)
      await page.fill('input[name="receiverAddress"], #receiverAddress', shippingAddress.address)
      await page.fill('input[name="receiverZipCode"], #receiverZipCode', shippingAddress.zipCode)

      // 주문 확인 클릭
      await page.click('button.order_confirm, #orderConfirmBtn, .confirm-order')
      await this.randomDelay()

      // 주문번호 추출
      const orderIdElement = await page.waitForSelector(
        '.order-number, #orderNumber, .order_id',
        { timeout: 10000 },
      )
      ...
      try {
        const page = await this.ensureBrowser()
        screenshotPath = `screenshots/order-fail-${request.sourceProductId}-${Date.now()}.png`
  ...
  async getTrackingNumber(wholesaleOrderId: string): Promise<string | null> {
    try {
      ...
      await page.goto(
        `${DOMAEGGUK_BASE_URL}/mypage/orders/${wholesaleOrderId}`,
        { waitUntil: 'domcontentloaded', timeout: 15000 },
      )
      await this.randomDelay()

      const trackingElement = await page.waitForSelector(
        '.tracking-number, #trackingNumber, .invoice_num',
        { timeout: 5000 },
      )
  ...
  buildUniqueKey(wholesaleOrderId: string): string {
    return `domaegguk-order:${wholesaleOrderId}`
  }
