...
async function main() {
    ...
    if (invalid.length > 0) {
        ...
        invalid.forEach((p) => {
            console.error(`  id=${p.id}  source=${p.source}  sourceProductId=${p.sourceProductId}  status=${p.status}  createdAt=${p.createdAt.toISOString()}`);
        });
...
//# sourceMappingURL=verify-unique-key.js.map